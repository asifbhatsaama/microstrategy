        ...
        for (let j in library){
            if (dossierID==library[j].target.id){
                let table = document.getElementById("favoriteTable");
                    ...
                    let cell5 = row.insertCell(4);
                    let cell6 = row.insertCell(5);
                    cell1.innerHTML = library[j].name;
                    cell2.innerHTML = library[j].target.id;
                    cell3.innerHTML = library[j].target.owner.name;
                    cell4.innerHTML = library[j].lastViewedTime;
                    cell5.innerHTML = library[j].projectId;
                    cell6.innerHTML = library[j].target.lastModifiedTime;
                    row.setAttribute("onmouseover", "style='background-color:#C0C0C0';");
                    row.setAttribute("onmouseout", "style='background-color:default';");
                $("#favoriteTable tr").click(function() {
                    var selected = $(this).hasClass("highlight");
                    $("#favoriteTable tr").removeClass("highlight");
                    if (!selected)
                        $(this).addClass("highlight");
                        sessionStorage.setItem("dossierid", document.querySelector(".highlight").children[1].innerHTML);
                        sessionStorage.setItem("projid", document.querySelector(".highlight").children[4].innerHTML);
